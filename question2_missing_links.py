...
with open('impressionNetwork.csv', 'r') as f:
    reader = csv.reader(f)
    ...
    for row in reader:
        for i in range(1,len(row)):
            if row[i] != 'none':   #if there is 'none' between two nodes then there is no edge between them. In case of no 'nan' then we will add an edge between them
